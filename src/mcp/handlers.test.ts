...
describe('MCPToolHandlers', () => {
  ...
  beforeEach(() => {
    ...
    mockExperienceHandler = (ExperienceHandler as jest.MockedClass<typeof ExperienceHandler>).mock.instances[0] as jest.Mocked<ExperienceHandler>;
    mockRecallHandler = (RecallHandler as jest.MockedClass<typeof RecallHandler>).mock.instances[0] as jest.Mocked<RecallHandler>;
    mockReconsiderHandler = (ReconsiderHandler as jest.MockedClass<typeof ReconsiderHandler>).mock.instances[0] as jest.Mocked<ReconsiderHandler>;
    mockReleaseHandler = (ReleaseHandler as jest.MockedClass<typeof ReleaseHandler>).mock.instances[0] as jest.Mocked<ReleaseHandler>;
  });
  ...
  describe('handle', () => {
    ...
    it('should delegate recall tool to RecallHandler', async () => {
      ...
      expect(mockRecallHandler.handle).toHaveBeenCalledWith(args);
      expect(result).toBe(expectedResult);
    });
    ...
    it('should delegate release tool to ReleaseHandler', async () => {
      ...
      expect(mockReleaseHandler.handle).toHaveBeenCalledWith(args);
      expect(result).toBe(expectedResult);
    });
  ...
  describe('error handling', () => {
    it('should handle null args', async () => {
      ...
      expect(mockExperienceHandler.handle).toHaveBeenCalledWith(null);
    ...
    it('should handle undefined args', async () => {
      ...
      expect(mockRecallHandler.handle).toHaveBeenCalledWith(undefined);
